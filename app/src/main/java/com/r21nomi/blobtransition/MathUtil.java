...
public class MathUtil {
    ...
    private static float lerp(float sourceValue1, float sourceValue2, float amount) {
        return sourceValue1 + (sourceValue2 - sourceValue1) * amount;
    }

//    static float ofMap(float value, float inputMin, float inputMax, float outputMin, float outputMax, boolean clamp) {
...
//    }
    ...
    private static float _slang_library_noise2(float x, float y) {
    ...
    private static float grad2( int hash, float x, float y ) {
        ...
        return ((h & 1) == 1 ? -u : u) + ((h & 2) == 0 ? -2.0f*v : 2.0f*v);
    }

    private static char perm[] = {151,160,137,91,90,15,
